...
export default function Designations() {
    ...
    const loadData = () => api.get('/designations').then(r => setDesignations(r.data));
    useEffect(() => { loadData(); }, []);

    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            if (editItem) await api.put(`/designations/${editItem.uuid}`, form);
    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <h2 style={{ fontSize: '22px', fontWeight: 700 }}>Designations</h2>
                        ...
                        {designations.map(d => (
                            <tr key={d.uuid}>
                                ...
                                <td style={{ display: 'flex', gap: '6px' }}>
                                    ...
                                    <button className="btn btn-ghost btn-sm" onClick={async () => { if (confirm('Delete?')) { await api.delete(`/designations/${d.uuid}`); loadData(); } }}><Trash2 size={14} /></button>
                                </td>
                            </tr>
                        ))}
                        {designations.length === 0 && <tr><td colSpan="3" style={{ textAlign: 'center', padding: '40px', color: 'var(--text-muted)' }}>No designations</td></tr>}
                    </tbody>
                </table>
            </div>
                ...
                <div className="modal-overlay" onClick={() => setShowModal(false)}>
                    <div className="modal" onClick={e => e.stopPropagation()}>
                        <div className="modal-header">
                            <h3 className="modal-title">{editItem ? 'Edit' : 'Add'} Designation</h3>
