...
export default function Tenants() {
    ...
    const getExpiryInfo = (expiry) => {
        ...
        if (diff < 0) return { text: `Expired (${Math.abs(diff)}d ago)`, color: '#ef4444', date: dayjs(expiry).format('DD MMM YYYY') };
        if (diff < 15) return { text: `${diff} days left`, color: '#f59e0b', date: dayjs(expiry).format('DD MMM YYYY') };
        return { text: `${diff} days left`, color: '#22c55e', date: dayjs(expiry).format('DD MMM YYYY') };
    };
